...
import { createMovieTags } from '@/movie/list/index';
...
export function createPagers(page, limit, total) {
    ...
    function createTag(text, status, targetPage) {
       ...
       // 传入的是disabled
       status === 'disabled' && span.addClass(styles.disabled);
       // 传入的是active
       status === 'active' && span.addClass(styles.active);
       if(status === ''){
           // 只有普通样式，才监听点击事件
    ...
    let pageNumber = Math.ceil(total / limit); // 最大页数
    //1. 创建首页标签
    createTag('首页', page === 1 ? 'disabled' : '', 1);
    //2. 创建上一页标签
    createTag('上一页', page === 1 ? 'disabled' : '', page - 1);
    //3. 创建数字页码标签
    const maxCount = 10; // 设置最大显示10页
